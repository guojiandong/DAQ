...
namespace Ksat.AppPlugIn.Communicate.IocpCore.Client
{
    /// <summary>
    /// udp客户端
    /// </summary>
    public class UdpClients
    {
        ...
        /// 需要发送的数据
        /// </summary>
        private ConcurrentQueue<Common.SendingQueue> sendQueue;
        /// <summary>
        ...
        private void Init()
        {
            sendQueue = new ConcurrentQueue<Common.SendingQueue>();
        }
        ...
        {
            sendQueue.Enqueue(new Common.SendingQueue() { remoteEndPoint = remoteEndPoint, data = data, offset = offset, length = length });
        }
