...
namespace Ksat.AppPlugIn.Communicate.IocpCore
{
    ...
    internal class TcpClients
    {
        ...
        internal TcpClients(int receiveBufferSize)
        {
            ...
            m_sendPool = new Common.SocketAsyncEventArgsPool(m_minSendSocketAsyncEventArgs);
            Init();
        }
        ...
        {
            sendQueue.Enqueue(new Common.SendingQueue() { data = data, offset = offset, length = length });
        }
        ...
        /// 发送数据到服务端
        /// </summary>
        internal void Send(Common.SendingQueue sendQuere)
        {
            if (!socket.Connected)
            {
                return;
