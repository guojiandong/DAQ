...
namespace Ksat.AppPlugIn.Communicate.IocpCore
{
    /// <summary>
    /// tcp Socket监听基库
    /// </summary>
    internal class TcpServer
    {
        ...
        /// </summary>
        private Common.BufferManager m_bufferManager;
        /// <summary>
        /// 用于监听传入连接请求的套接字
        /// </summary>
        ...
        /// </summary>
        private Common.SocketAsyncEventArgsPool m_receivePool;
        /// <summary>
        /// 发送端SocketAsyncEventArgs对象重用池，发送套接字操作
        /// </summary>
        private Common.SocketAsyncEventArgsPool m_sendPool;
        /// <summary>
        ...
        /// 已经连接的对象池
        /// </summary>
        internal ConcurrentDictionary<int, Common.ConnectClient> connectClient;
        /// <summary>
        /// 发送线程数
        /// </summary>
        ...
        /// 需要发送的数据
        /// </summary>
        private ConcurrentQueue<Common.SendingQueue>[] sendQueues;
        /// <summary>
        /// 锁
        /// </summary>
        ...
        internal TcpServer(int numConnections, int receiveBufferSize, int overTime)
        {
            ...
            m_bufferManager = new Common.BufferManager(receiveBufferSize * m_numConnections, receiveBufferSize);
            m_receivePool = new Common.SocketAsyncEventArgsPool(m_numConnections);
            m_sendPool = new Common.SocketAsyncEventArgsPool(m_numConnections);
        ...
        private void Init()
        {
            connectClient = new ConcurrentDictionary<int, Common.ConnectClient>();
            sendQueues = new ConcurrentQueue<Common.SendingQueue>[sendthread];
            for (int i = 0; i < sendthread; i++)
            {
                sendQueues[i] = new ConcurrentQueue<Common.SendingQueue>();
            }
        ...
        private void ProcessAccept(SocketAsyncEventArgs e)
        {
            connectId++;
            //把连接到的客户端信息添加到集合中
            Common.ConnectClient connecttoken = new Common.ConnectClient();
        ...
        {
            sendQueues[connectId % sendthread].Enqueue(new Common.SendingQueue() { connectId = connectId, data = data, offset = offset, length = length });
        }
        ...
        private void Send(Common.SendingQueue sendQuere)
        {
            Common.ConnectClient client;
