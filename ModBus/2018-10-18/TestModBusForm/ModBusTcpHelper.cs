...
namespace TestModBusForm
{
    class ModBusTcpHelper
    {
        ...
        {
            byte[] addressByte = Ksat.AppPlugIn.Utils.BinaryUtil.HexToByte(hex_address.Trim());
            byte addr_Start_Hi = addressByte[0];
            byte addr_Start_Lo = addressByte[1];

            byte[] lengthByte = Ksat.AppPlugIn.Utils.BinaryUtil.HexToByte(hex_length.Trim());
            ...
            ModBusTcpDataFrameMessage msg = new ModBusTcpDataFrameMessage(buffer, 0);
            return msg.ToMsg();
        }
        ...
        /// <param name="hex_length">需要写线圈的个数 "00 0A"</param>
        /// <param name="bin_data">长度"1001111010"</param>
        public static byte[] BuildWriteMultiCoilCommand(string hex_address, string coil_count_hex, string bin_data)
        {
            ...
            byte[] lengthByte = Ksat.AppPlugIn.Utils.BinaryUtil.HexToByte(coil_count_hex.Trim());
            byte length_Hi = lengthByte[0];
            byte length_Lo = lengthByte[1];
            ...
            byte[] binary_data = MobBusUtil.GetByteArr(bin_data.Trim(), 8);// 一个线圈 占一个byte
        ...
        public static byte[] BuildWriteOneRegisterCommand(string hex_address, byte[] values)
        {
            ...
            ModBusTcpDataFrameMessage msg = new ModBusTcpDataFrameMessage(data, 0);
            return msg.ToMsg();
        }
        ...
        public static byte[] BuildWriteMultiRegisterCommand(string hex_address, string reigster_count_hex, byte[] values)
        {
            ...
            byte[] registerCountByte = Ksat.AppPlugIn.Utils.BinaryUtil.HexToByte(reigster_count_hex.Trim());
            ...
            byte[] data = new byte[] { 0x90, 0x91, 0x00, 0x00, message_byte[1], message_byte[0], 0xff, 0x10, addr_Start_Hi, addr_Start_Lo, registerCount_Hi, registerCount_Lo, (byte)(register_count_length * 2) };
            Buffer.BlockCopy(data, 0, buffer, 0, data.Length);
