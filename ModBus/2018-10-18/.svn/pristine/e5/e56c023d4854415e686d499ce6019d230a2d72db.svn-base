...
namespace Ksat.AppPlugIn.Communicate.FastIocp.Client
{
    ...
    public class SocketClient<TMessage> : Base.AbstractSessionBase
        where TMessage : class, Base.Messaging.IMessage
    {
        ...
        /// </summary>
        public event Action<Base.IConnection, TMessage> ReceivedUnknowMessage;
        #endregion
        ...
        public SocketClient(Base.Protocol.IProtocol<TMessage> protocol)
        ...
        public bool TryRegisterEndPoint(string name, EndPoint[] arrRemoteEP, Func<Base.IConnection, Task> initFunc = null)
        {
        ...
        {
            return this._endPointManager.CheckExisting(name);
        }
        ...
        public void Send(Request<TMessage> request)
        {
            ...
            Base.IConnection connection = null;
        ...
        public bool Send(Base.Packet packet)
        {
            ...
            if (!this._connectionPool.TryAcquire(out connection))
            {
                sLog.Warn("Send() send packet failed:"+packet.Tag+", seqid:"+packet.SeqID);
                return false;
            }
        ...
        /// <exception cref="ArgumentNullException">request is null.</exception>
        public void Send(Base.IConnection connection, Request<TMessage> request)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public override int NextRequestSeqID()
        {
        ...
        /// <param name="name"></param>
        /// <param name="connection"></param>
        protected virtual void OnEndPointConnected(string name, Base.IConnection connection)
        {
        ...
        /// <param name="name"></param>
        /// <param name="connection"></param>
        protected virtual void OnEndPointAlready(string name, Base.IConnection connection)
        {
        ...
        /// <param name="connection"></param>
        /// <param name="request"></param>
        protected virtual void OnSent(Base.IConnection connection, Request<TMessage> request)
        {
        }
        /// <summary>
        /// on send failed
        ...
        protected virtual void OnReceived(Base.IConnection connection, Request<TMessage> request, TMessage message)
        {
            ThreadPool.QueueUserWorkItem(_ =>
            {
        ...
        /// <param name="connection"></param>
        /// <param name="message"></param>
        protected virtual void OnReceivedUnknowMessage(Base.IConnection connection, TMessage message)
        {
        ...
        protected override void OnDisconnected(Base.IConnection connection, Exception ex)
        {
            if(ex == null)
            {
                this.UnRegisterEndPoint(connection.Tag);
            }
        ...
        /// <param name="connection"></param>
        /// <param name="packet"></param>
        protected override void OnStartSending(Base.IConnection connection, Base.Packet packet)
        {
        ...
        protected override void OnSendCallback(Base.IConnection connection, Base.Packet packet, bool isSuccess)
        {
            ...
            if (isSuccess)
            {
                request.SentTime = Utils.DateHelper.UtcNow;
            ...
            if (Utils.DateHelper.UtcNow.Subtract(request.CreatedTime).TotalMilliseconds > this._millisecondsSendTimeout)
            {
                //send time out
        ...
        private class PendingSendQueue
        {
            ...
            public PendingSendQueue(SocketClient<TMessage> client)
            {
                ...
                this._timer = new Timer(state =>
                {
                    ...
                    var dtNow = Utils.DateHelper.UtcNow;
        ...
        private class NodeInfo
        {
            ...
            /// init function
            /// </summary>
            public readonly Func<Base.IConnection, Task> InitFunc;
            #endregion
            ...
            public NodeInfo(string name, EndPoint[] arrRemoteEP,
                Func<Base.IConnection, Task> initFunc)
            {
        ...
        private class Node : IDisposable
        {
            ...
            private readonly Action<Node, Base.IConnection> _connectedCallback;
            private readonly Action<Node, Base.IConnection> _alreadyCallback;

            private bool _isdisposed = false;
            private Base.IConnection _connection = null;
            ...
            public Node(NodeInfo info, Base.ISession host,
                Action<Node, Base.IConnection> connectedCallback,
                Action<Node, Base.IConnection> alreadyCallback)
            {
            ...
            private void ConnectCallback(Task<Socket> t)
            {
                if (t.IsFaulted)
                {
                    ...
                    Utils.TaskHelper.Delay(new Random().Next(700, 2200)).ContinueWith(_ => this.Connect());
                    return;
                }
                ...
                connection.Disconnected += (conn, ex) =>
                {
                    ...
                    Utils.TaskHelper.Delay(new Random().Next(1000, 3000)).ContinueWith(_ => this.Connect());
                };
        ...
        private class EndPointManager
        {
            ...
            /// </summary>
            public event Action<string, Base.IConnection> Connected;
            /// <summary>
            /// node already event
            /// </summary>
            public event Action<string, Base.IConnection> Already;
            #endregion
            ...
            /// </summary>
            /// <param name="host"></param>
            public EndPointManager(Base.ISession host)
            {
                this._host = host;
            }
            #endregion
            ...
            /// <param name="initFunc"></param>
            /// <returns></returns>
            public bool TryRegister(string name, EndPoint[] arrRemoteEP, Func<Base.IConnection, Task> initFunc)
            {
                lock (lockObject)
                {
            ...
            public bool CheckExisting(string name)
            {
                lock (lockObject)
                {
                    return this._dicNodeInfo.ContainsKey(name);
                }
            }
            /// <summary>
            /// to array
            ...
            /// <param name="connection"></param>
            private void OnNodeConnected(Node node, Base.IConnection connection)
            {
                if (this.Connected == null) return;

                connection.Tag = node.Info.Name;
            ...
            /// <param name="node"></param>
            /// <param name="connection"></param>
            private void OnNodeAlready(Node node, Base.IConnection connection)
            {
        ...
        private interface IConnectionPool
        {
            ...
            /// </summary>
            /// <param name="connection"></param>
            void Register(Base.IConnection connection);
            /// <summary>
            /// try acquire <see cref="SocketBase.IConnection"/>
            /// </summary>
            /// <param name="connection"></param>
            /// <returns></returns>
            bool TryAcquire(out Base.IConnection connection);
            /// <summary>
            /// release
            /// </summary>
            /// <param name="connection"></param>
            void Release(Base.IConnection connection);
            /// <summary>
            /// destroy
            /// </summary>
            /// <param name="connection"></param>
            void Destroy(Base.IConnection connection);
            #endregion
        }
        ...
        {
            #region Private Members
            private readonly List<Base.IConnection> _list = new List<Base.IConnection>();
            private Base.IConnection[] _arr = null;
            ...
            /// </summary>
            /// <param name="connection"></param>
            public void Register(Base.IConnection connection)
            {
                if (connection == null) throw new ArgumentNullException("connection");
            ...
            /// <param name="connection"></param>
            /// <returns></returns>
            public bool TryAcquire(out Base.IConnection connection)
            {
                var arr = this._arr;
            ...
            /// </summary>
            /// <param name="connection"></param>
            public void Release(Base.IConnection connection)
            {
            }
            /// <summary>
            /// destroy
            /// </summary>
            /// <param name="connection"></param>
            public void Destroy(Base.IConnection connection)
            {
                if (connection == null) throw new ArgumentNullException("connection");
        ...
        {
            #region Private Members
            private readonly ConcurrentDictionary<long, Base.IConnection> _dic =
                new ConcurrentDictionary<long, Base.IConnection>();
            private readonly ConcurrentStack<Base.IConnection> _stack =
                new ConcurrentStack<Base.IConnection>();
            #endregion
